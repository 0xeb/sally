...
extern std::wstring AssignedFSNameFTPS;
...
extern LPCWSTR SAVEBITS_CLASSNAME; // class for CWaitWindow
...
extern std::wstring TargetPanelPath;

extern std::wstring UserDefinedSuffix; // preloaded suffix for user-defined server types
...
extern const wchar_t* CONFIG_SERVERTYPES;
extern const wchar_t* CONFIG_STNAME;
extern const wchar_t* CONFIG_STADCOND;
extern const wchar_t* CONFIG_STCOLUMNS;
extern const wchar_t* CONFIG_STRULESFORPARS;
extern const wchar_t* CONFIG_FTPSERVERLIST;
extern const wchar_t* CONFIG_FTPPROXYLIST;
extern const wchar_t* CONFIG_FTPPRXUID;
extern const wchar_t* CONFIG_FTPPRXNAME;
extern const wchar_t* CONFIG_FTPPRXTYPE;
extern const wchar_t* CONFIG_FTPPRXHOST;
extern const wchar_t* CONFIG_FTPPRXPORT;
extern const wchar_t* CONFIG_FTPPRXUSER;
extern const wchar_t* CONFIG_FTPPRXPASSWD_OLD;
extern const wchar_t* CONFIG_FTPPRXPASSWD_SCRAMBLED;
extern const wchar_t* CONFIG_FTPPRXPASSWD_ENCRYPTED;

extern const wchar_t* CONFIG_FTPPRXSCRIPT;
...
// Temporary adapter for narrow formatting/protocol callers; new text code uses LangStr.
...
// Native-wide password-manager boundary. Encrypted payloads remain opaque bytes;
// plaintext is never projected through the process ACP.
...
class CFTPSecureByteStringGuard
{
public:
    explicit CFTPSecureByteStringGuard(std::string& value) noexcept : Value(value) {}
    ~CFTPSecureByteStringGuard() noexcept { FTPSecureWipe(Value); }

    CFTPSecureByteStringGuard(const CFTPSecureByteStringGuard&) = delete;
    CFTPSecureByteStringGuard& operator=(const CFTPSecureByteStringGuard&) = delete;

private:
    std::string& Value;
};

class CFTPSecureWideStringGuard
{
public:
    explicit CFTPSecureWideStringGuard(std::wstring& value) noexcept : Value(value) {}
    ~CFTPSecureWideStringGuard() noexcept { FTPSecureWipe(Value); }
...
// Stores the path to Documents in dynamically owned UTF-16; leaves it empty on failure.
BOOL GetMyDocumentsPathW(std::wstring& initDir) noexcept;

// Formats a localized system error into dynamically owned UTF-16 and contains
// allocation failures at the UI boundary.
void FTPShowSystemError(HWND parent, int formatResID, DWORD error) noexcept;

// Decodes local configuration bytes and transactionally publishes the complete
// user-visible name (without '*', with the user-defined suffix when applicable).
...
void LoadPassword(HKEY regKey, CSalamanderRegistryAbstract* registry, const wchar_t* oldPwdName, const wchar_t* scrambledPwdName,
                  const wchar_t* encryptedPwdName, BYTE** encryptedPassword, int* encryptedPasswordSize);
...
public:
    ...
    char* ID;                     // encoded column identifier for the parser
    int NameID;                   // string identifier for the name (-1 = use a non-standard (non-translated) name - see 'NameStr')
    char* NameStr;                // encoded non-standard name (if NameID is not -1, this is NULL)
    ...
    char* DescrStr;               // encoded non-standard description (if DescrID is not -1, this is NULL)
    CSrvTypeColumnTypes Type;     // type of values in the column
    char* EmptyValue;             // encoded parser value to use when no value is parsed (NULL == standard empty value)
...
public:
    ...
    // Transactionally stores the complete encoded parser record without an arbitrary capacity.
    BOOL SaveToStr(std::string& text, BOOL ignoreColWidths = FALSE) noexcept;

    // allocates a copy of the structure, returns NULL on error
    CSrvTypeColumn* MakeCopy();

    // sets the structure
    void Set(BOOL visible, const char* id, int nameID, const char* nameStr, int descrID,
...
protected:
    ...
    // resulting string in 'result'; returns FALSE on allocation error, otherwise TRUE
    BOOL LoadStr(const char** str, char** result);
};
...
struct CServerType
{
    // server type name; if the name starts with '*',
    ...
    //
    char* TypeName;
    char* AutodetectCond;                   // encoded condition used for autodetection; NULL = always true
    TIndirectArray<CSrvTypeColumn> Columns; // definition of columns for parsing and for the panel
    char* RulesForParsing;                  // encoded rules for parsing listings; NULL = empty
...
struct CFTPServer
{
    std::wstring ItemName;           // bookmark label (for the Bookmarks list box in the Connect dialog)
    std::wstring Address;            // semantic host/IP, dynamically owned UTF-16
    std::wstring InitialPath;        // semantic initial remote path; encoded only after session negotiation
    int AnonymousConnection;         // TRUE = anonymous login, FALSE = UserName and Password apply
    std::wstring UserName;           // semantic user name; empty and non-anonymous means ask the user
    BYTE* EncryptedPassword;         // password; if empty (or NULL) and not an anonymous login, ask the user
    ...
    std::wstring TargetPanelPath;    // local target-panel path; empty means no requested path change
    std::string ServerType;          // local-config bytes for the listing server type; empty means auto-detect
    ...
    // sets the structure, returns FALSE on error
    BOOL Set(const wchar_t* itemName,
    ...
    {
        return Set(src.ItemName.c_str(),
                   src.Address.c_str(),
                   src.InitialPath.c_str(),
                   ...
                   src.SavePassword,
                   src.ProxyServerUID,
                   src.TargetPanelPath.c_str(),
                   src.ServerType.empty() ? NULL : src.ServerType.c_str(),
                   src.TransferMode,
                   src.Port,
                   src.UsePassiveMode,
...
public:
    ...
    BOOL AddServer(const wchar_t* itemName,
                   const wchar_t* address = NULL,
                   const wchar_t* initialPath = NULL,
                   ...
                   const wchar_t* targetPanelPath = NULL,
...
{
    std::wstring ProxyHost;                // semantic proxy server address (empty = ask the user)
    int ProxyPort;                         // proxy server port (always defined)
    std::wstring ProxyUser;                // semantic proxy user name (empty = unknown, skip the whole line)
    std::wstring ProxyPassword;            // semantic proxy password (empty = ask the user)
    std::wstring Host;                     // semantic FTP server address (always defined)
    int Port;                              // FTP server port (always defined)
    std::wstring User;                     // semantic FTP user name (empty = ask the user)
    std::wstring Password;                 // semantic FTP password (empty = ask the user)
    std::wstring Account;                  // semantic FTP account (empty = ask the user)
    ...
    BOOL Valid;              // FALSE only when dynamic semantic credential ownership could not be created

    BOOL NeedUserInput() { return NeedProxyHost || NeedProxyPassword || NeedUser || NeedPassword || NeedAccount; }
    BOOL IsGood() const { return Valid; }

    // 'host' cannot be NULL
    CProxyScriptParams(CFTPProxyServer* proxyServer, const wchar_t* host,
...
// the function returns TRUE if the script is OK (FALSE on error, with the text in 'errorDescription');
...
//   description is in 'errorDescription'
...
//   and returns the connection target in 'connectionHost'+'port'; '*execPoint' points to the start
...
//   TRUE and 'sendCommand' contains the command (including CRLF at the end), 'logCommand' contains the
//   log text (the password is replaced with "(hidden)"); '*execPoint' points to the start
//   of the next script line
// - end of script (not possible at the beginning of the script): returns TRUE and 'sendCommand' is an empty
//   string; '*execPoint' points to the end of the script;
// 'connectionHost' (if not NULL) is dynamically owned semantic UTF-16; command outputs are
// dynamically owned encoded protocol bytes and are published transactionally;
// 'errorDescription' receives a dynamically owned local-text description; 'proxyHostNeeded' (if not NULL)
// returns TRUE if ProxyHost must be specified (we connect to it); 'lowMemory' distinguishes
// allocation/size failure from script syntax and missing-input results.
BOOL ProcessProxyScript(const CFtpTextCodec& textCodec,
                        ...
                        BOOL* proxyHostNeeded, BOOL* lowMemory = NULL) noexcept;
...
// Returns an owned proxy-server type label for the inherited byte log or native-wide UI.
...
struct CFTPProxyForDataCon // data about the proxy server for the data connection
{
    ...
    std::wstring ProxyUser;        // semantic proxy user name; encoded only by the socket adapter
    std::wstring ProxyPassword;    // semantic proxy password; wiped by the owner
    std::wstring Host;             // semantic FTP server address
    DWORD HostIP;                  // FTP server IP address
    unsigned short HostPort;       // FTP server port
    BOOL Valid;                    // all requested encoded byte owners were allocated
...
struct CFTPProxyServer
{
    ...
    std::wstring ProxyName; // unique (case insensitive) proxy-profile label in combo boxes

    CFTPProxyServerType ProxyType; // proxy server type

    std::wstring ProxyHost;         // semantic proxy server address/IP
    int ProxyPort;                  // proxy server port
    std::wstring ProxyUser;         // semantic user name for logging in to the proxy server
    ...
    std::wstring ProxyPlainPassword; // decrypted semantic password; filled from ProxyEncryptedPassword and wiped on release
    int SaveProxyPassword;          // TRUE = password should be stored on media (registry/config file)

    std::string ProxyScript; // explicitly encoded script for custom proxy connections
    ...
    CFTPProxyForDataCon* AllocProxyForDataCon(DWORD proxyHostIP, const wchar_t* host,
                                              DWORD hostIP, unsigned short hostPort);

    // sets ProxyHost
    BOOL SetProxyHost(const wchar_t* proxyHost) noexcept;

    // sets ProxyPort
    void SetProxyPort(int proxyPort);

    // sets ProxyPlainPassword
    BOOL SetProxyPassword(const wchar_t* proxyPassword) noexcept;

    // sets ProxyUser
    BOOL SetProxyUser(const wchar_t* proxyUser) noexcept;
    ...
    BOOL Set(const CFTPProxyServer& src)
    {
        return Set(src.ProxyUID,
                   ...
                   src.ProxyScript.empty() ? NULL : src.ProxyScript.c_str());
    }
...
public:
    ...
    BOOL IsProxyNameOK(CFTPProxyServer* proxyServer, const wchar_t* proxyName);
    ...
    BOOL GetProxyName(std::wstring& name, int proxyServerUID);
    ...
    CFTPProxyServer* MakeCopyOfProxyServer(int proxyServerUID, BOOL* lowMem) noexcept;
...
public:
    ...
    std::wstring CommandHistory[COMMAND_HISTORY_SIZE]; // semantic history of user-entered FTP commands
    int SendSecretCommand;                      // last state of the "Secret command" checkbox in the "Send FTP Command" dialog

    std::wstring HostAddressHistory[HOSTADDRESS_HISTORY_SIZE]; // semantic server-address history
    std::wstring InitPathHistory[INITIALPATH_HISTORY_SIZE]; // semantic initial-path history
...
protected:                             // data used from multiple threads, access is synchronized:
    ...
    std::wstring AnonymousPasswd;            // password for anonymous connection (user's e-mail)
...
public:
    ...
    BOOL GetAnonymousPasswd(std::wstring& password) noexcept;
    BOOL SetAnonymousPasswd(const wchar_t* password) noexcept;
...
public:
    ...
    virtual void WINAPI GetFileDataForUpDir(const wchar_t* archivePath, CFileData& upDir) {}          // FS does not use
    virtual BOOL WINAPI GetFileDataForNewDir(const wchar_t* dirName, CFileData& dir) { return TRUE; } // FS does not use
...
protected:
    ...
    DWORD RawNameOffset;                     // exact server bytes for the parsed item name
    CFtpTextCodec TextCodec;                  // negotiated codec captured for presentation
...
public:
    ...
    BOOL StoreRawName(CFileData& file, const char* name, size_t length);
    const char* GetRawName(const CFileData& file) const;
    BOOL GetWireName(const CFileData& file, const CFtpTextCodec& codec, std::string& name) const;
    BOOL DecodeTextForPresentation(std::string_view bytes, std::wstring& text) const noexcept;
    ...
    // a pointer to the name and extension. Dynamically owned 'storage' keeps an adjusted
    // VMS name alive for the caller. Returns FALSE only on allocation failure.
    BOOL GetBasicName(const CFileData& file, const wchar_t** name, const wchar_t** ext,
                      std::wstring& storage) noexcept;
...
{
protected:
    std::wstring Host;        // semantic host (FTP server)
    int Port;                 // port on which the FTP server runs
    std::wstring User;        // semantic FTP user identity
    std::string Path; // current path on FTP in encoded session bytes (note that it is case-sensitive)
    ...
    std::string RescuePath;                      // rescue path on FTP in encoded session bytes - try when ChangePath() can no longer shorten the path
    std::string HomeDir;                         // default path on FTP in encoded session bytes - set after logging into the server
    ...
    std::optional<std::string> PathListing;      // explicitly encoded listing bytes; disengaged until at least an empty listing was obtained
    ...
    int DirLineHotPathUserLength;          // helper variable: WCHAR length of the current username if it contains "disallowed" characters
    ...
    BOOL AutodetectSrvType;          // TRUE = autodetection should be used to determine the server type
    std::string LastServerType;      // dynamically owned local-config bytes, without optional leading '*'
...
public:
    ...
    // Builds a live UTF-16 identity while decoding only the remote path bytes
    // from the FTP session domain.
    BOOL BuildUserPartText(std::wstring& userPart, const char* path = NULL,
                           BOOL ignorePath = FALSE) const;
    BOOL BuildFullPathText(const wchar_t* fsName, const char* path,
                           std::wstring& fullPath) const;
    BOOL DecodeSessionBytes(std::string_view bytes, std::wstring& text) const;
    BOOL EncodeSessionText(const wchar_t* text, std::string& bytes) const;
    BOOL GetEncodedSessionUserLength(int& userLength) const;
    BOOL ResolveFullServerPathBytes(HWND parent, std::string& path, BOOL& success);
    BOOL ChangePathBytes(int currentFSNameIndex, std::wstring& fsName, int fsNameIndex,
                         const wchar_t* userPartW, const char* userPart,
    ...
    BOOL ParseListing(CSalamanderDirectoryAbstract* dir, CPluginDataInterfaceAbstract** pluginData,
                      CServerType* serverType, BOOL* lowMem, BOOL isVMS, const wchar_t* findName,
    ...
    void ClearHostFromListingCacheIfFirstCon(const wchar_t* host, int port, const wchar_t* user);

    // determines whether this FS connects to 'host'+'port' as 'user'
    BOOL ContainsHost(const wchar_t* host, int port, const wchar_t* user);
    ...
    virtual BOOL WINAPI GetPathForMainWindowTitle(const wchar_t* fsName, int mode, CSalamanderStringBuffer* buf);
