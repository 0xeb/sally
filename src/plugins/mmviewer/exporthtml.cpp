...
static std::string HTMLConvertToUTF8(const char* chars, bool bUTF8 = false);

// Wide overload for item NAMES. This is strictly better than what the narrow
// path does for them: the label goes straight from UTF-16 to UTF-8, instead of through CP_ACP
// on the way - so an export can now carry a label the ANSI codepage cannot represent.
static std::string HTMLConvertToUTF8(const wchar_t* wide, bool = false)
{
    std::string utf8;
    if (wide == NULL || !Win32EncodeText(CP_UTF8, wide, wcslen(wide), utf8))
        return std::string();
    return HTMLConvertToUTF8(utf8.c_str(), true);
}

static std::string HTMLConvertToUTF8(const char* chars, bool bUTF8)
{
    if (chars == NULL)
        return std::string();
    ...
    {
        std::wstring wide;
        if (!Win32DecodeText(CP_ACP, chars, strlen(chars), wide) ||
            !Win32EncodeText(CP_UTF8, wide, utf8))
            return std::string();
    }
...
static void AppendText(TDirectArray<char>& output, const std::string& text)
{
    if (!text.empty())
        output.Add(text.data(), static_cast<int>(text.size()));
}

static void WriteHTMLHeader(TDirectArray<char>& output, const wchar_t* title)
{
    AppendText(output,
...
static void WriteHTMLTableStart(TDirectArray<char>& output, const wchar_t* title, BOOL superHeader = FALSE)
{
...
static void WriteHTMLTableLine(TDirectArray<char>& output, const wchar_t* name, const char* value, bool bUTF8)
{
...
int ExportToHTML(const wchar_t* fname, COutput& Output)
{
    ...
    WriteHTMLHeader(text, LangStr(IDS_AUDIOINFO).c_str());
