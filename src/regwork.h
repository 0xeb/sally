...
BOOL OpenKeyAux(HWND parent, HKEY hKey, const wchar_t* name, HKEY& openedKey, BOOL quiet = TRUE);
BOOL CreateKeyAux(HWND parent, HKEY hKey, const wchar_t* name, HKEY& createdKey, BOOL quiet = TRUE);
BOOL GetValueAux(HWND parent, HKEY hKey, const wchar_t* name, DWORD type, void* buffer,
...
public:
    ...
    BOOL CreateKey(HKEY key, const wchar_t* name, HKEY& createdKey);

    // opens the existing subkey 'name' of key 'key'; returns the handle in 'openedKey'
    // and TRUE on success. The returned key ('createdKey') must be closed by calling the CloseKey.
    BOOL OpenKey(HKEY key, const wchar_t* name, HKEY& openedKey);
    ...
    BOOL DeleteKey(HKEY key, const wchar_t* name);

    // loads the value of 'name' + 'type' + 'buffer' + 'bufferSize' from the key 'key'; returns success
    BOOL GetValue(HKEY key, const wchar_t* name, DWORD type, void* buffer, DWORD bufferSize);
    ...
    // returns success
    BOOL SetValue(HKEY key, const wchar_t* name, DWORD type, const void* data, DWORD dataSize);
    ...
    BOOL GetSize(HKEY key, const wchar_t* name, DWORD type, DWORD& bufferSize);
