...
public:
    ...
    // The ANSI convenience overloads are DELETED: every caller
    // passes wide text now, so an accidental narrow argument is a compile error
    // instead of a silent CP_ACP round-trip.
};
